...
Deno.serve(async (req: Request) => {
  ...
  const INTERNAL_ACTIONS = new Set(["images_generate", "images_start", "images_status", "images_pdf", "integration_app_token"]);
  ...
  try {
    switch (action) {
      // ───── Integration.app / Membrane customer access token ─────
      case "integration_app_token": {
        const workspaceKey = Deno.env.get("INTEGRATION_APP_WORKSPACE_KEY") ?? Deno.env.get("MEMBRANE_WORKSPACE_KEY");
        const workspaceSecret = Deno.env.get("INTEGRATION_APP_WORKSPACE_SECRET") ?? Deno.env.get("MEMBRANE_WORKSPACE_SECRET");
        if (!workspaceKey || !workspaceSecret) {
          return err("Integration.app workspace credentials missing", 500);
        }
