...
function isUngradedSetupChoice(card: any): boolean {
  ...
  if (/\b(ungraded|preference|preferences|config|configuration|setup|exam_setup|survey|choice|depth|time_budget)\b/.test(explicit)) {
    return true;
  }
  ...
  const looksLikeLearningSetup =
    /(exam|quiz|test|mock|practice|lesson|session|study|training|drill|revision|review|minutes?|mins?|quick|regular|focused|deep|easy|medium|hard|beginner|intermediate|advanced)/i.test(`${question} ${options}`) ||
    /(امتحان|اختبار|تدريب|مراجعة|درس|تعليم|دراسة|أسئلة|دقيقة|دقائق|سريع|عادي|مركز|مركّز|عميق|سهل|متوسط|صعب|مبتدئ|متقدم)/i.test(`${question} ${options}`);

  return asksPreference && looksLikeLearningSetup;
}
...
const MCQCard = ({ card, onAnswer }: BaseProps) => {
  ...
  const numericCorrect = typeof card.correct === "number" ? card.correct : Number(card.correct);
  const correct = Number.isFinite(numericCorrect)
    ? Math.min(Math.max(Math.trunc(numericCorrect), 0), Math.max(options.length - 1, 0))
    : 0;
  const isRight = !ungradedSetupChoice && picked === correct;
  ...
  const submit = (i: number) => {
    ...
    if (ungradedSetupChoice) {
      hapticFeedback("tap");
      const payload = `[LEARN_CHOICE] type=setup_choice question="${escapeLearnField(card.question)}" chosen="${escapeLearnField(`${chosenLetter}) ${options[i]}`)}" topic="${escapeLearnField(cardTopic(card) || card.topic || card.subject || "")}"`;
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        {options.map((opt: string, i: number) => {
          ...
          if (revealed && ungradedSetupChoice) {
            if (isPicked) {
