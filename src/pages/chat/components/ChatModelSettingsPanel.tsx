...
export function ChatModelSettingsPanel() {
  const [preferences, setPreferences] = useState<ChatModelPreferences>({
    effort: "medium",
    deepThinking: false,
  });
  ...
  return (
    <div className="space-y-3">
        ...
        {EFFORTS.map(({ id, label, description, badge }, index) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 py-3 text-start ${index ? "border-t border-foreground/10" : ""}`}
            >
      ...
      <p className="px-2 text-[11px] leading-relaxed text-foreground/50">
        Higher effort produces more thorough answers, but may take longer.
      </p>
      <div className="flex items-center gap-3 rounded-2xl bg-foreground/[0.045] px-4 py-3.5">
        ...
        <button
          ...
          className={`relative h-7 w-12 shrink-0 rounded-full transition-colors ${preferences.deepThinking ? "bg-primary" : "bg-foreground/15"}`}
        >
